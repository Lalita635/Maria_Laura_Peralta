class Game {
    ...
    showWelcome() {
        Swal.fire("Bienvenido", `Divertite jugando!
<br>
<strong>Controles:</strong>
<ul class="list-group">
<li class="list-group-item"> <kbd>P</kbd><br>Pausar o Reanudar </li>
<li class="list-group-item"> <kbd>Espacio</kbd><br>Rotar </li>
<li class="list-group-item"> <kbd>Flechas de dirección</kbd><br>Mover figuras </li>
<li class="list-group-item">También podes usar los <strong>botones</strong> (útiles si tenes pantalla táctil) </li>
</ul>
...
`);
    ...
    playerLoses() {
        // Compruebe si hay algo en Y 1. Tal vez no sea justo para el jugador, pero funciona.
    ...
    mainLoop() {
        ...
        // Si la figura puede moverse hacia abajo, muévase hacia abajo
        if (this.figureCanMoveDown()) {
            this.globalY++;
        } else {
            // Si la figura no puede, entonces comenzamos un tiempo de espera porque
            // el jugador puede mover la figura para mantenerlo bajando
            // por ejemplo cuando la figura colapsa con otros puntos pero queda
            // espacio a la izquierda o a la derecha y el jugador se mueve allí para que la figura pueda seguir bajando
            if (this.timeoutFlag) return;
            this.timeoutFlag = true;
            setTimeout(() => {
                this.timeoutFlag = false;
                // Si el tiempo expira, volvemos a verificar si no puede seguir bajando.
                // Si puede (porque el jugador lo movió), regresamos y mantenemos el ciclo.
                if (this.figureCanMoveDown()) {
                    return;
                }
                // En este punto, sabemos que la figura colapsó con el piso o con otro punto.
                // Entonces movemos toda la figura a la matriz de piezas existente.
    ...
    /**
     *
     * @param point Un objeto que tiene propiedades x e y; las coordenadas no deben ser globales, sino relativas al punto
     * @returns {boolean}
     */
